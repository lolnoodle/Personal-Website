  ...
  return (
    <div className="min-h-dvh bg-[var(--background)] flex flex-col">
      <div className="px-8 pt-10 pb-8 border-b border-[var(--border)]">
        ...
        <p className="font-space text-sm text-[var(--text)] opacity-40 mt-2 tracking-widest">
          {journals.length} {journals.length === 1 ? "entry" : "entries"} in the
          setlist
        </p>
      </div>
          ...
          <motion.div key={journal.slug} variants={itemVariants}>
            ...
            >
              ...
              <div className="flex flex-col gap-2 flex-1 min-w-0">
                ...
                <p className="font-space text-sm leading-relaxed text-[var(--text)] opacity-50">
                  {journal.description}
                </p>

                <div className="flex items-center justify-between mt-1">
                  <div className="flex gap-2 flex-wrap">
                    <span className="font-space text-xs text-[var(--text)] opacity-40 tracking-widest">
                      {journal.date}
                    </span>
                    {journal.tags.map((tag) => (
                      <span
                        key={tag}
                        className="font-space text-xs text-[var(--accent)] border border-[var(--accent)] opacity-70 px-1.5 py-0.5 tracking-widest"
                      >
                        {tag}
                      </span>
                    ))}
                  </div>
                  <span className="font-space text-xs text-[var(--text)] opacity-40 tracking-widest flex-shrink-0">
                    {journal.readTime}
                  </span>
                </div>
              </div>
